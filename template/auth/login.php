...
<head>
    ...
    <title>login</title>
    <link rel="shortcut icon" href="http://localhost/restaurant/icon" type="image/x-icon" />
...
<section class="container-fluid bg-auth-form d-flex justify-content-center align-items-center ">
    <section class="row bg-gradiant-green-blue">
        ...
        <section class="col-12 bg-white p-5 rounded">
            ...
            <form method="post" class="" action="http://localhost/restaurant/check-login">
                <?php
                $httpReferer = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : null;
                if($httpReferer == 'http://localhost/restaurant/login'){?>
...
<?php if($httpReferer == 'http://localhost/restaurant/login'){?>
    swal({
        title: "Login Error!",
